...
int main(int argc, char *argv[]){
	...
	if(argc != 5){
		...
		printf("./blur IMAGE_SIZE_X IMAGE_SIZE_Y BLOCK_SIZE_X BLOCK_SIZE_Y\n");
		printf("For example:\n./blur 1000 10000 16 16\n");
		return 1;
	}

	IMAGE_SIZE_X = atoi(argv[1]);
	IMAGE_SIZE_Y = atoi(argv[2]);
	...
	grid_size_x = (IMAGE_SIZE_X - 1) / block_size_x + 1; //// +1 for last datas
...
//		                |                             |   <--(IMAGE_SIZE_X + IMAGE_PAD_SIZE)
//			            | ''''''''''''''''''''''''''  |                                                           ^
//		                | :                         : |                            (IMAGE_SIZE_Y / stream_count)  |
//		                | :                         : |                                                           +
//                      | ..........................  |
//		                |_____________________________|   <-- (IMAGE_SIZE_X + IMAGE_PAD_SIZE)
//		
//			
//	
	for(int i = 0; i < stream_count; i++){
		...
		int stream_bytes_h_to_d = (stream_size + stream_pad_size) * sizeof(input_d[0]);   ///size of data from host to device          
		int stream_bytes_d_to_h = (stream_size) * sizeof(input_d[0]);                     ///size of data from device to host            
		int offset = i * stream_size;
		int copy_offset = (i > 0)? i * (IMAGE_SIZE_Y / stream_count) * 
			(IMAGE_SIZE_X + IMAGE_PAD_SIZE): offset;

		cudaMemcpyAsync(&input_d[copy_offset], &input_h[copy_offset], stream_bytes_h_to_d, cudaMemcpyHostToDevice, streams[i % STREAM_NUMBERS]);   //copy data from host to device
		
		int stream_y = IMAGE_SIZE_Y / stream_count;

		blur_kernel_shared_memory_revised_II<<< grid_dime, block_dime, shared_memory_size, streams[i % STREAM_NUMBERS]>>>(&output_d[offset], &input_d[copy_offset], stream_y, IMAGE_SIZE_X, pad);   //set the size of blocks, grids, sharedMemory

		cudaMemcpyAsync(&device_output_h[offset], &output_d[offset], stream_bytes_d_to_h, cudaMemcpyDeviceToHost, streams[i % STREAM_NUMBERS]);  //copy data from device to host
	}
